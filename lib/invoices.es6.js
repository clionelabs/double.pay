...
D.Invoice.transform = (doc) => {
  doc.timeBasedItems = _.map(doc.timeBasedItems, function(timeBasedItem) {
    return _.extend(timeBasedItem, D.Invoice.TimeBasedItem.ProtoType);
  });
  var invoice = _.extend(doc, D.Invoice.ProtoType);
  return invoice;
};

D.Invoices = new Meteor.Collection('invoices', {
