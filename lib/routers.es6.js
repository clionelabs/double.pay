...
Router.route('invoices/:invoiceId', {
  ...
  data() {
    ...
    let data = Invoices.findOne({ _id : invoiceId });
    const currentCustomer = data ? Users.findOneCustomer(data.customerId, { fields : Users.showDisplayOnlyOptions() }) : null;
...
Router.route('authorization/:customerId', {
  ...
  action() {
    ...
    if (authorizationInfo) {
      this.render('paymentNewAuthorization');
    }
  }
});

Router.route('authorized/:customerId', {
  name: 'authorized',
  data() {
    return Session.get('authorizedInfo');
  },
  onRun() {
    Session.set('authorizedInfo', null);
    let customerId = this.params.customerId;
    Meteor.call('getPaymentAuthorizedInfo', customerId, function(error, result) {
      Session.set('authorizedInfo', result);
    });
    this.next();
  },
  action() {
    let authorizedInfo = Session.get('authorizedInfo');
    if (authorizedInfo) {
      this.render('paymentAuthorized');
    }
  }
});
